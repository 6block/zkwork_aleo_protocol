...
impl<T: FromBytes + ToBytes + Send + 'static> Data<T> {
 ...
 pub enum PoolMessageSC<N: Network> {
     /// ConnectAck := (is_accecpt, address, [id])
     ConnectAck(bool, Address<N>, Option<u32>),
 ...
 impl<N: Network> PoolMessageSC<N> {
     ...
     pub fn serialize_data_into<W: Write>(&self, writer: &mut W) -> Result<()> {
         match self {
             Self::ConnectAck(is_accept, address, id) => match is_accept {
                 true => match id {
                     Some(id) => {
                     ...
                     None => return Err(anyhow!("ConnectAck: Invalid id")),
                 },
                 false => {
     ...
     pub fn deserialize(buffer: &[u8]) -> Result<Self> {
         ...
         let message = match id {
             0 => match data.is_empty() {
                 ...
                 false => match data[0] {
                     0 => Self::ConnectAck(false, bincode::deserialize(&data[1..=32])?, None),
                     1 => Self::ConnectAck(
                         true,
 ...
 pub enum PoolMessageCS<N: Network> {
     ...
     Submit(u32, u64, Address<N>, Data<ProverSolution<N>>),
     /// DisConnect := (id)
     DisConnect(u32),
     // Unused
     #[allow(unused)]
 ...
 impl<N: Network> PoolMessageCS<N> {
     ...
     pub fn serialize_data_into<W: Write>(&self, writer: &mut W) -> Result<()> {
         match self {
             ...
             Self::Submit(worker_id, job_id, address, prover_solution) => {
     ...
     pub fn deserialize(buffer: &[u8]) -> Result<Self> {
         ...
         let message = match id {
             ...
             129 => Self::Submit(
                 ...
                 bincode::deserialize(&data[12..44])?,
                 Data::Buffer(data[44..].to_vec().into()),
             ),
 ...
 mod tests {
     ...
     fn check_pool_message_sc(message: PoolMessageSC<CurrentNetwork>) {
         ...
         let _ = PoolMessageSC::<CurrentNetwork>::default().encode(message1.clone(), &mut buffer_2);
     ...
     fn check_pool_message_cs(message: PoolMessageCS<CurrentNetwork>) {
         ...
         let mut buffer_2 = BytesMut::new();
         let _ = PoolMessageCS::default().encode(message1.clone(), &mut buffer_2);
     ...
     fn test_pool_message_sc() -> Result<()> {
         ...
         let message = PoolMessageSC::ConnectAck::<CurrentNetwork>(true, address, Some(1));
     ...
     fn test_pool_message_cs() -> Result<()> {
         let message = PoolMessageCS::Connect::<CurrentNetwork>(0, 0, 1, 0, "my_worker_1".to_string());
         ...
         let message = PoolMessageCS::Submit::<CurrentNetwork>(0, 0, address, Data::Object(prover_solution));
         check_pool_message_cs(message);
